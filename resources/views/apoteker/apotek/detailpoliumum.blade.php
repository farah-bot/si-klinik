...
@section('content')
    <div class="data-pengguna-container">
        <div class="container">
            ...
            <div class="form-group">
                <button type="submit" class="btn btn-tambah" id="simpan">Simpan</button>
        ...
        <script>
            ...
            $(document).ready(function() {
                $('#simpan').click(function(event) {
                    ...
                    $.ajax({
                        url: '{{ route('updateStatusUmumAntrian', $pemeriksaan->kunjungan->id) }}',
                        type: 'PUT',
                        data: {
                            _token: '{{ csrf_token() }}',
                            status_antrian: 'Sudah Terlayani'
                        },
                        success: function(response) {
                            if (response.success) {
                                window.location.href = '{{ route('dataapotek') }}';
                            } else {
                                console.error('Gagal memperbarui status antrian.');
                            }
                        },
                        error: function(xhr, status, error) {
                            console.error('Error updating status antrian:', error);
                        }
                    });
                });
            });
